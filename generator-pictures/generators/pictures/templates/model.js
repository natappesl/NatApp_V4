...
<%# NOTE: There are three instances of this function in the code. Changes must be copied over to all of them %>        processImage = function(img) {
        <% if (source === 'everlive' || source === 'sitefinity' ) { %>
        ...
        <% } %>
            ...
            }<% if (source === 'everlive') { %> else if (typeof img === 'string' && !isAbsolute(img)) {
                var setup = dataProvider.setup || {};
                img = setup.scheme + ':' + setup.url + setup.appId + '/Files/' + img + '/Download';
            }<% } %>
        <% if (source === 'sitefinity') { %>
            if (dataProvider && typeof img === 'string' && !isAbsolute(img)) {
                var url = dataProvider.profileUrl,
...
<% } %>
        ...
        transport: {<% if (source === 'everlive') { %>
            ...
            read: {
                url: dataProvider.url<% if (source === 'json') { %>,
                dataType: 'json'<% } %>
            }<% } %>
    ...
    parent.set('onShow', function (e) {
        ...
        if (!<%= name %>.get('dataSource')) {
            dataSource = new kendo.data.DataSource(dataSourceOptions);
            <%= name %>.set('dataSource', dataSource);<% if (isMapView) {%>
